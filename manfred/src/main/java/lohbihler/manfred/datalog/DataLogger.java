...
public interface DataLogger {
	...
	/**
	 * The initialization gives the data logger an opportunity to set itself up
	 * in preparation for logging. Data loggers are initially disabled, and do
	 * not start logging until start() is called by an enabler.
	 * 
	...
	/**
	 * Called by the enabled to indicate that the logger should begin storing
	 * data.
	 */
	public void start() throws Exception;

	/**
	 * Called by the system at shutdown to indicate that logging should stop and
	...
	default void createEnabler(FlightSample flightRegister, GpsSample gpsRegister, ScheduledExecutorService timer,
		...
		if (enablerProps == null) {
			...
			try {
				...
				LOGGER.info("Enabler initialized");
			} catch (Exception e) {
				LOGGER.error("Error initializing enabler", e);
